...
type LeaderWorkerSetReconciler struct {
	...
	broadcaster chan event.GenericEvent
}
...
func NewLeaderWorkerSetReconciler(client client.Client, scheme *runtime.Scheme, record record.EventRecorder, broadcaster chan event.GenericEvent) *LeaderWorkerSetReconciler {
...
//   - When sts is rolling update, the partition will start from the last index to the 0 index step by 1.
...
func (r *LeaderWorkerSetReconciler) rollPartition(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet) (int32, error) {
	...
	if partition == 0 {
		// If template not updated but size did,
		// we'll broadcast the event to pod conciliation
		// for worker statefulsets rolling updates.
		r.broadcastIfSizeUpdated(ctx, lws)
		return 0, nil
	}
...
func (r *LeaderWorkerSetReconciler) broadcastIfSizeUpdated(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet) {
	stsSelector := client.MatchingLabels(map[string]string{
		leaderworkerset.SetNameLabelKey: lws.Name,
	})
	var stsList appsv1.StatefulSetList
	...
	// To trigger leader pod conciliation.
	for _, sts := range stsList.Items {
		if sts.Name == lws.Name {
			continue
		}
		if *sts.Spec.Replicas != *lws.Spec.LeaderWorkerTemplate.Size-1 {
			r.broadcaster <- event.GenericEvent{Object: &sts}
		}
	}
}
